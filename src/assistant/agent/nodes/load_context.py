...
def load_context(state: AgentState, deps: AgentDeps) -> dict:
    ...
    Reading both fresh every turn is what makes persona hot-edits and preference changes
    take effect on the very next question. A one-off preference ("...as bullets just this
    once") is carried in ``oneoff_preference`` and applied at report time by
    ``compose_system_prompt`` for this turn only — never written back (plan/010 §2).
    """
